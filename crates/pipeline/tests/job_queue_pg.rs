...
async fn enqueue_creates_job_row() -> anyhow::Result<()> {
    ...
    let id = s
        .queue
        .enqueue(s.tenant_id, None, JobKind::Ingest, 100)
        .await?;
...
async fn enqueue_allows_file_id() -> anyhow::Result<()> {
    ...
    let id = s
        .queue
        .enqueue(s.tenant_id, Some(42), JobKind::Ingest, 50)
        .await?;
...
async fn enqueue_preserves_kind_and_priority() -> anyhow::Result<()> {
    ...
    let id = s
        .queue
        .enqueue(s.tenant_id, None, JobKind::Reembed, 25)
        .await?;
...
async fn claim_respects_priority_ordering() -> anyhow::Result<()> {
    ...
    let low_id = s
        .queue
        .enqueue(s.tenant_id, None, JobKind::Ingest, 10)
        .await?;
    let mid_id = s
        .queue
        .enqueue(s.tenant_id, None, JobKind::Ingest, 50)
        .await?;
    let high_id = s
        .queue
        .enqueue(s.tenant_id, None, JobKind::Ingest, 100)
...
async fn concurrent_claimants_never_get_the_same_row() -> anyhow::Result<()> {
    ...
    for i in 0..50 {
        let id = s
            .queue
            .enqueue(s.tenant_id, None, JobKind::Ingest, i)
            .await?;
        ids.push(id);
    }
